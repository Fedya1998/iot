...
import queue
def check_input(inp):
    super_list = ['\"data\"', '\"temperature\"', '\"humidity\"', '\"pressure\"', '\"status\"', '\"devEUI\"', '\"rssi\"', '\"temperature\"', '\"battery\"',
                  '\"date\"']
    ...
    good_keys1 = sorted(['humidity', 'pressure', 'temperature'])
    ...
    for i in range(len(good_keys0)):
        if sorted(list(msg.keys()))[i] != good_keys0[i]:
            ...
            return False
    #if len(sorted(list(msg.keys()))) > len(good_keys0):
    #    print("Bad keys:")
    #    for i in range(len(sorted(list(msg.keys()))) - len(good_keys0)):
    #        print(sorted(list(msg.keys()))[i])
    #    return False
    ...
    #if len(sorted(list(msg['data'].keys()))) > len(good_keys1):
    #    print("Bad keys:")
    #    for i in range(len(sorted(list(msg['data'].keys()))) - len(good_keys1)):
    #        print(sorted(list(msg['data'].keys()))[i])
    #    return False
    ...
    for item in ['temperature', 'humidity', 'pressure']:
